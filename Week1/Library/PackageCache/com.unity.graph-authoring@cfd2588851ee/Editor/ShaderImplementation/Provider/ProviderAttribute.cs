...
namespace Unity.GraphAuthoring.Editor.ProviderSystem
{
    ...
    internal static class ProviderTypeCache
    {
        ...
        internal static void PopulateLibrary(ProviderLibrary lib)
        {
            foreach (var type in GetScriptedProviderTypes())
                if (Activator.CreateInstance(type, true) is IProvider provider)
                    lib.TryAdd(provider);
        }
    }
}
