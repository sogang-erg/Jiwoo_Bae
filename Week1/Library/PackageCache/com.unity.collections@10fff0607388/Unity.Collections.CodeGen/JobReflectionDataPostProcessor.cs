    ...
    internal partial class JobsILPostProcessor : ILPostProcessor
    {
        ...
        private bool GenerateCalls(TypeReference producerRef, TypeReference jobStructType, MethodBody body, ILProcessor processor)
        {
            try
            {
                ...
                // Make sure the job type is accessable to the generated CreateJobReflectionData at the assembly root
                MakeTypeAccessableToAssembly(jobStructType);

                // Make sure any types used as GenericArguments are also accessable
                if (jobStructType is GenericInstanceType genericInstance)
                {
                    foreach (var g in genericInstance.GenericArguments)
                    {
                        MakeTypeAccessableToAssembly(g);
                    }
                }
        ...
        static void MakeTypeAccessableToAssembly(TypeReference type)
        {
            while (type != null)
            {
                var typeDefinition = type.CheckedResolve();

                if (typeDefinition.IsNestedPrivate || typeDefinition.IsNestedFamily)
                {
                    typeDefinition.IsNestedAssembly = true;
                }

                type = typeDefinition.DeclaringType;
            }
        }
