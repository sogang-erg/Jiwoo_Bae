...
* `Add` methods to `UnsafeParallelHashMap.ParallelWriter` and `NativeParallelHashMap.ParallelWriter`.
* `TryAdd` methods to `NativeHashSet`, `NativeParallelHashSet`, `UnsafeHashSet`, and `UnsafeParallelHashSet` (and their `ParallelWriters`, where applicable) to allow user-code to better describe its intent.
* `ReadOnly/Span<T>` support for `UnsafeList`, `NativeList`, and `FixedList`.
* SortIndices method to allow sorting large structures without reordering them in memory.
...
* Obsolete `NativeList.GetInternalListDataPtrUnchecked` extension method, use `NativeList.GetUnsafeList` instead.
...
* **Behaviour Breaking Change:** Previously, `UnsafeParallelHashMap.Add` did not return a bool, but it also didn't throw - it silently failed. It now correctly throws an `ArgumentException` if the given key was already present. Prefer `TryAdd` if graceful recovery is intended.
* Clarified documentation regarding collection `UnionWith` helpers, and to note that `ParallelWriter.Add` and `TryAdd` methods will throw if the collection is full.
* Issue where DataStreamReader failed to deserialize ulong variables only on Android, with burst off. Affected Unity 6.0.38 and onwards.
...
* NativeReference bad assignment of safety handle to wrong container.
