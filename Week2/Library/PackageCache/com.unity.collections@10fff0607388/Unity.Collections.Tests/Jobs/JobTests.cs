...
namespace Unity.Jobs.Tests.ManagedJobs
{
    ...
    internal class JobTests : JobTestsFixture
    {
        ...
        // EarlyJobInit is generated for all jobs in assemblies which in refrerence Unity.Collections. Depening on the jobs accesability modifier (public, private, internal)
        // these generated might be inaccessable at their callsite in CreateJobReflectionData this causes issues on CoreCLR which enforces accesability correctnes at runtime.
        // Here we test for those failuers. Errors are produced at script compile time (before the test runs) so we capture all the logs from startup using JobTestsLogLogger
        // and check them for any errors containing known problematic job types each defined in thier own assembly since the compiler only emits one error message per assembly
        static readonly Type[] k_JobsTypesToTest = { Collections.Tests.CoreCLR.TestJobs.NestedPrivateTestJobContainterClass.TestJobType,
                                                     Collections.Tests.CoreCLR.TestJobs.NestedProtectedTestJobContainterClass.TestJobType,
                                                     Collections.Tests.CoreCLR.TestJobs.NestedPublicGenericTestJobContainterClass.TestJobType,
                                                     Collections.Tests.CoreCLR.TestJobs.InternalJobNestedPrivateContainerClass.TestJobType};

        [TestCaseSource(nameof(k_JobsTypesToTest))]
        public void CheckGeneratedEarlyJobInitAccess(Type jobType)
        {
#if !UNITY_EDITOR
            JobTestsLogLogger.StopCapture();
            foreach (var l in JobTestsLogLogger.logs)
            {
                if (l.logType == LogType.Exception)
                {
                    Assert.False(l.message.Contains(GetFriendlyName(jobType)), $"Encountered an exception containing one of our known problematic types {GetFriendlyName(jobType)}");
                }
            }
#endif
        }
...
// I'm not completely convinced by this like its OK but do we need in playmode tests too its quite heavy and it gets added everywhere we probably don't want that
#if !UNITY_EDITOR
    static internal class JobTestsLogLogger
    {
        ...
        static List<SavedLog> _logs = new List<SavedLog>();

        public static List<SavedLog> logs{ get{ return _logs; } }

        static void OnLog(string condition, string stacktrace, LogType type)
        {
            lock (_logs)
            {
                _logs.Add(new SavedLog { message = "Log " + condition, callstack = stacktrace, logType = type });
            }
        }
